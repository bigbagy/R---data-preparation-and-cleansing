...
**A**: 
Finding 1: The duration of movies fall into two major categories, short movies (runtime 0-30mins) and long movies (runtime approx 50-150mins)
Finding 2: Short movie runtime remain alomist constant over the years, however long movei runtime has positive correlation with year, long movie runtime tends to increase as year progresses
Finding 3: The budget generally increases as the year progresses
 ...
 #step2. for each movie, need to check 3 pairs of year values for consistency:
  ...
  if (!(is.na(year) ||  is.na(release))){
    ...
    if (( (date+1) < year ) || ((date-1) > year) )
      rmlist<-c(rmlist,i)}
#step3. record to-be-removed row numbers in "rmlist"", "rmlist"" contains list of rows to be removed
